...
// My Skills 초기화 함수를 전역 스코프에 정의
function initMySkills() {
  const mySkillsSection = document.querySelector(".my-skills");
  if (!mySkillsSection) return;
  ...
  function animateSkills() {
    ...
    circles.forEach((circle) => {
      ...
      gsap.to(circle, {
        background: `conic-gradient(#00d9ff ${percentage * 3.6}deg, #1e2a38 0deg)`,
        duration: 1.5,
        ease: "power2.out",
        scrollTrigger: {
          trigger: circle,
    ...
    // 제목 애니메이션
    const titles = mySkillsSection.querySelectorAll("h1, h2");
  ...
  // ScrollTrigger로 섹션 진입 시 애니메이션 시작
  ScrollTrigger.create({
