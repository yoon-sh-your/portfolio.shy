...
document.querySelectorAll(".title h1[data-number]").forEach((h1) => {
  ...
  numberArray.forEach((num) => {
    ...
    img.src = `../../common_project/common/img/title_heading_${num}.svg`;
    img.alt = `숫자 ${num}`;
...
document.querySelectorAll("h2[data-type]").forEach((h2) => {
  ...
  if (digit) {
    ...
    img.src = `../../common_project/common/img/title_mid_${digit}.svg`; // 이미지 파일명 구성
...
document.querySelectorAll("h2[data-icon]").forEach((h2) => {
  ...
  if (iconName) {
    const img = document.createElement("img");
    img.src = `../../common_project/common/img/icon_${iconName}.svg`; // 이미지 파일명 구성
...
document.querySelectorAll("h2[data-head]").forEach((h2) => {
  ...
  if (headNumber) {
    const img = document.createElement("img");
    img.src = `../../common_project/common/img/title_heading_num_${headNumber}${isStar ? "_star" : ""}.svg`;
...
document.querySelectorAll("h2").forEach((h2) => {
  ...
  if (hasFractoinBox.length > 0) {
    ...
    const headerStyle = window.getComputedStyle(h2);
    const minHeight = parseInt(headerStyle.getPropertyValue("min-height"));
    if (height <= minHeight) {
