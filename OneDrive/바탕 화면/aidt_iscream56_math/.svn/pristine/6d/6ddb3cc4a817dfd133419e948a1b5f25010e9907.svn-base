...
function onIncorrectTwice() {
    ...
    page.querySelectorAll(".input_wrap math-field:not(.textarea), .input_wrap math-field.textarea, .custom_dropdown, .drawing_grid_area").forEach((wrapper) => {
        ...
        if (isDrawingGridArea) {
            if (wrapper.dataset.correction === "false") {
                wrapper.classList.add("hint");
            } else if (wrapper.dataset.correction === "true") {
                // 정답인 격자 영역 처리
                wrapper.classList.add("correct");
                wrapper.classList.remove("hint");
            }
        } else {
            // 기존 input, dropdown 처리
            const inner = wrapper.querySelector("input, textarea, select.custom_dropdown, .connection_lines"); // 이 부분은 drawing_grid_area와 무관
...
observeAttributeChange(".btn_area .btnReset, .btn_area .btnCheck, .btn_area .btnSample, .btn_area .btnSubmit", "class", (target) => {
    ...
    const submitBtn = document.querySelector(".btn_area .btnSubmit");
    const resetBtn = document.querySelectorAll(".btn_area .btnReset");
    const revealBtns = page.querySelectorAll(".reveal_btn");
    const hasOtherAnswers = page.querySelectorAll("[data-answer-single]:not(.reveal_btn)").length > 0;
    const letKnow = page.querySelectorAll(".letKnow li");
    const letCheck = page.querySelectorAll(".letCheck li");

    // ❗ page_completed 상태는 watchWithCustomTest에서 처리하므로 여기서는 제거
    // if(page.classList.contains("completed")){
    //     return;
    // }

    if (revealBtns.length > 0 && !hasOtherAnswers || letKnow.length > 0 ||letCheck.length > 0) {
        return;  
    }

    // // 모든 버튼이 active가 없을 때만 리셋 버튼 비활성화 (❗ completed 조건 제거)
    // if (!checkBtn?.classList.contains("active") && !sampleBtn?.classList.contains("active") && !submitBtn?.classList.contains("active")) {
...
observeAttributeChange(".connect_wrap", "data-connections", (wrap) => {
    ...
    if (answerSingle !== undefined) {
        try {
            answerSingleValue = JSON.parse(answerSingle || "[]");
...
if(buttonSupplies.length > 0){
    buttonSupplies.forEach(btn => {
        btn.addEventListener('click', function(e){
            ...
            const suppliesClose = supplies.querySelector("button.close");
            suppliesClose.addEventListener('click', function(){
