...
document.querySelectorAll(".drawing_area").forEach(drawingArea => {
  ...
  function createBentLine(startDot, endDot, area, svg) {
    ...
    hintPath.setAttribute("stroke-width", "20"); // 클릭 영역을 넓히기 위해 두께를 20으로 설정
    hintPath.setAttribute("fill", "none");
    hintPath.classList.add("hint_line", `hint_${connectionCount}`);
    ...
    hintPath.addEventListener("click", () => {
      ...
      // 연결된 점들 다시 사용 가능하도록 설정
      const parent1 = parseInt(startDot.dataset.index);
      const parent2 = parseInt(endDot.dataset.index);

      // 생성된 점과 라벨 제거
      const generatedDot = area.querySelector(`.dot[data-index="${newLabel}"]`);
      const connectionLabel = area.querySelector(`.connection_label[data-label-index="${newLabel}"]`);

      if (generatedDot) {
        // 생성된 점과 연결된 모든 선과 점 제거
        removeConnectedLines(newLabel, area, svg);
        generatedDot.remove();
        availableDots.delete(parseInt(generatedDot.dataset.index));
      }
      if (connectionLabel) {
        connectionLabel.remove();
      }

      // 원래 점들을 다시 사용 가능하도록 설정
      availableDots.set(parent1, startDot);
      ...
      userConnections = userConnections.filter(conn =>
        !(conn[0] === parent1 && conn[1] === parent2) &&
        !(conn[0] === parent2 && conn[1] === parent1)
      );
      svg.dataset.userConnections = JSON.stringify(userConnections);

      // 해당 선의 bend 정보 제거
      existingBends = existingBends.filter(bend =>
        !(bend.startX === startX && bend.endX === endX && bend.y === midY)
      );
  ...
  function removeConnectedLines(generatedDotIndex, area, svg) {
    ...
    const connectedPaths = Array.from(svg.querySelectorAll('.connection_line')).filter(path => {
      const pathConnections = Array.from(svg.querySelectorAll(`.connection_label[data-label-index="${generatedDotIndex}"]`));
      return pathConnections.length > 0;
    });

    // 연결된 선들과 관련된 점들 찾기
    const connectedDots = new Set();
    connectedPaths.forEach(path => {
      const label = area.querySelector(`.connection_label[data-label-index="${generatedDotIndex}"]`);
      if (label) {
        const dot = area.querySelector(`.dot[data-index="${label.dataset.labelIndex}"]`);
        if (dot) {
    ...
    // 연결된 선들 제거 및 bend 정보 제거
    connectedPaths.forEach(path => {
      // 선 제거 전에 해당 선의 bend 정보도 제거
      const pathData = path.getAttribute('d');
      if (pathData) {
        // path 데이터에서 좌표 추출
        const matches = pathData.match(/M\s+(\d+)\s+(\d+)\s+V\s+(\d+)\s+H\s+(\d+)\s+H\s+(\d+)\s+V\s+(\d+)/);
        if (matches) {
          const [_, startX, startY, midY, midX, endX, endY] = matches.map(Number);
          // 해당 좌표와 일치하는 bend 정보 제거
          existingBends = existingBends.filter(bend =>
            !(bend.startX === startX && bend.endX === endX && bend.y === midY)
          );
        }
    ...
    connectedDots.forEach(dot => {
      ...
      const label = area.querySelector(`.connection_label[data-label-index="${dotIndex}"]`);
      if (label) {
        label.remove();
      }
      dot.remove();
      availableDots.delete(parseInt(dotIndex));
    });
  }
...
style.textContent = `
  ...
  .hint_line:hover + .connection_line {
    stroke: #0066cc;
    filter: drop-shadow(0 0 2px rgba(0, 102, 204, 0.5));
  }
`;
