runAfterAppReady(() => {
	...
	$draggable.draggable({
	  ...
	  stop: function (event, ui) {
		// Use a small timeout to ensure drop event handlers have completed
		setTimeout(function () {
	...
	$droppable.droppable({
	  ...
	  drop: function (event, ui) {
		...
		// Make the dropped element draggable again for removal
		$droppedEl.draggable({
		  revert: "invalid",
		  revertDuration: 0,
		  ...
		  stop: function (event, ui) {
			...
			// Calculate center using ui.offset and element size
			...
			// Calculate droppable bounds using its rect
			...
			if (isOutside) {
			  // Parse current data-value again
			  let valuePartsStop = (
			  ...
			  // Update data-value: set the corresponding part to false
			  ...
			  // Snap back inside the droppable area
		...
		// Append the new draggable element
		$this.append($droppedEl);
		audioManager.playSound("drop");
		checkAnswer(); // Check answer after successful drop
	  },
	});
