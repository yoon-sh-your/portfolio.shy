...
async function getScreenImage(selector) {
    ...
    // 1. 복제된 라디오 버튼의 name 속성 변경 (그룹 분리)
    const clonedRadios = clone.querySelectorAll('input[type="radio"]');
    if (clonedRadios.length > 0) {
        clonedRadios.forEach(radio => {
            // 원래 name에 '-clone' 접미사 추가하여 새로운 그룹 생성
            radio.name = radio.name + '-clone';
        });

        // 2. 원본 라디오 버튼의 체크 상태 저장
        const originalRadios = target.querySelectorAll('input[type="radio"]');
        const checkedStates = Array.from(originalRadios).map(radio => {
        return {
            name: radio.name,
            value: radio.value,
        ...
        // 3. 복제본에 체크 상태 적용 (변경된 name 고려)
        checkedStates?.forEach(state => {
            if (state.checked) {
                const matchedClonedRadio = clone.querySelector(`input[type="radio"][name="${state.name}-clone"][value="${state.value}"]`);
                if (matchedClonedRadio) {
                matchedClonedRadio.checked = true;
                }
            }
        });
    }    
...
function initSendResultEvent() {
    $("#app_wrap .video_wrap .play, .btn-play-video .play")
        .off()
        .on("click", function () {
            var result = {};
            result.caliper = {
    ...
    $("#app_wrap .btn_area .btnReset").on("click", function () {
        var result = {};
        result.caliper = {
            "EVENT_TYPE": "NavigationEvent",
            "PROFILE_TYPE": "AssessmentProfile",
    ...
    $("#app_wrap .btn_area .btnSubmit, #app_wrap .btnCheck").on("click", async function () {
        var type = $(this).attr("class").split(" ")[0];

        // 체크일 경우 카운트가 1이 아닐 경우에만 전송
        if(type === 'btnCheck' && globalFaultCount === 1) {
            return;
        }
        var result = {};
        result.caliper = {
...
function inputDataSet(page, pageIndex) {
    var $input_result = [];
    $input_result[0] = [];
    $input_result[1] = [];
    $input_result[2] = [];
    ...
    if (page !== undefined) {
        ...
        // 정오답 여부
        $input_result[5][0] = globalIsCorrect[pageIndex];
    } else {
        // 단일 페이지
        ...
        // 정오답 여부
        $input_result[5][0] = globalIsCorrect[pagenation.currentPage-1];
    }
