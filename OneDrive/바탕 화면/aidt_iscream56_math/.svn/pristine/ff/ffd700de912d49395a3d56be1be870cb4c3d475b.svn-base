runAfterAppReady(function () {
    ...
    window.resetCustom = function () {
        ...
        const activePage = pagenation.activePage;
        if (activePage) {
            const activeGraphDiv = activePage.querySelector('.circle-graph-division');
            if (activeGraphDiv && activeGraphDiv._circleGraphInstance) {
                activeGraphDiv._circleGraphInstance.reset();
            }
        }
    }
    ...
    window.onIncorrectTwiceCustom = function () {
        // console.log("onIncorrectTwiceCustom");
...
class CircleGraph {
    constructor(graphDiv) {
        ...
        this.svg = graphDiv.querySelector('svg');
        this.NS = "http://www.w3.org/2000/svg";

        // 인스턴스 저장
        graphDiv._circleGraphInstance = this;
    ...
    reset() {
        ...
        // 버튼 상태 업데이트 강제 실행 (필요한 경우)
        if (window.forceWatchEvaluation) {
            window.forceWatchEvaluation();
        }
    }
