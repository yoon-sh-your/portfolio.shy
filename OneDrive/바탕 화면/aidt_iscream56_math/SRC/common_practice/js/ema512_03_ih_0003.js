...
runAfterAppReady(() => {
      ...
      drop: function(event, ui) {
        ...
        ui.helper.data("dropped", true);
          ...
          if ($this.find(`.${typeClass}`).length > 0) {
              // 드래그 드롭 실패 시에는 소리 재생 (클릭 실패는 클릭 핸들러에서 처리)
          ...
          $droppedEl.draggable({
              ...
              stop: function(event, ui) {
                  ...
                  const isOutside = droppedCenterX < droppableLeft || droppedCenterX > droppableRight ||
                                    droppedCenterY < droppableTop || droppedCenterY > droppableBottom;
  ...
  $draggable.off('click').on('click', function(e) {
      ...
      if (selectedDraggable && selectedDraggable[0] === this) {
          ...
          // 다른 것이 선택되어 있었거나 아무것도 선택되지 않았을 때: 새로 선택
          if (selectedDraggable) {
              selectedDraggable.removeClass('selected');
          }
      ...
      updateCheckoutHighlights(true); // 하이라이트 업데이트
  });
  ...
  $droppable.off('click').on('click', function(e) {
      ...
      if (selectedDraggable) {
          // 선택된 draggable 아이템이 있을 때
          const $dragItem = selectedDraggable; // 선택된 아이템
          const isTriangle = $dragItem.hasClass("triangle");
          const isCircle = $dragItem.hasClass("circle");
          const typeClass = isTriangle ? "triangle" : (isCircle ? "circle" : null);

          if (!typeClass) return; // 혹시 모를 오류 방지
  ...
  // 외부 클릭 시 선택 해제 (추가)
  $(document).off("click.checkoutDragDrop03ih0003").on("click.checkoutDragDrop03ih0003", function(e) {
