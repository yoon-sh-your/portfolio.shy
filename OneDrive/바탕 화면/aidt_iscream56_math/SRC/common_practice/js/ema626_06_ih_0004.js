runAfterAppReady(() => {
	...
	// 외부 클릭 시 선택 해제
	$(document)
		.off("click.checkoutDragDrop")
		.on("click.checkoutDragDrop", function (e) {
			if (
				selectedDraggable &&
				!$(e.target).closest(".drag_checkout .draggable").length &&
				!$(e.target).closest(".drag_checkout .droppable").length
			) {
				selectedDraggable.removeClass("selected");
				selectedDraggable = null;
				updateCheckoutHighlights(false);
