...
function renderProblem(problemStr) {
    const [left, right] = problemStr.split("÷").map((s) => s.trim());
    const leftHTML = renderFraction(left);
    const rightHTML = renderFraction(right);
    return `${leftHTML} <span style="margin:0 10px;">÷</span> ${rightHTML}`;
}
cardEl.textContent = currentProblem;
cardEl.innerHTML = renderProblem(currentProblem);
...
toggleCardBtn.addEventListener("click", (event) => {
    ...
    totalResetBtn.classList.add("active");
});
...
mathResetBtn.addEventListener("click", (e) => {
    ...
    mathCheckBtn.disabled = true;
    mathResetBtn.disabled = true;
});
...
startBtn.addEventListener("click", (e) => {
    if (startBtn.textContent === "확인") {
        ...
        const mathFields = document.querySelectorAll(".input_wrap math-field");
        mathFields.forEach((field) => {
...
function checkMathInputs() {
    const v1 = cardInput1.getValue("latex-expanded").trim();
    const v2 = cardInput2.getValue("latex-expanded").trim();
    const v3 = answerInput.getValue("latex-expanded").trim();
    const hasInput = v1 !== "" || v2 !== "" || v3 !== "";

    mathCheckBtn.disabled = !hasInput;
    mathResetBtn.disabled = !hasInput;
    btnSubmit.classList.add("active");
}

cardInput1.addEventListener("input", checkMathInputs);
cardInput2.addEventListener("input", checkMathInputs);
answerInput.addEventListener("input", checkMathInputs);
