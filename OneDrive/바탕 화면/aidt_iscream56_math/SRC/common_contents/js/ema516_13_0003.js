...
runAfterAppReady(() => {
  ...
  window.onIncorrectTwiceCustom = function () {
    ...
    currentPage.querySelectorAll(".trapezoid").forEach((el) => {
  ...
  window.resetCustom = function () {
    ...
    $(currentPage).find(".trapezoid").removeClass("hidden");
    $(currentPage).find(".answer_img").removeClass("active");
    $(currentPage).find(".trapezoid .line").removeClass("active");
  };
  ...
  defineButtonClassRules([
    {
      ...
      test: (el) => {
        ...
        if (!currentPage) return false;
        // 버튼 활성화 조건
        const val = $(el).closest(".trapezoid").find(".line.active");
  ...
  $(document).on("click", ".trapezoid .line", function (e) {
    e.stopPropagation();
