runAfterAppReady(() => {
  ...
  window.customCheckCondition = function (el) {
    ...
    if (!activePage || !activePage.classList.contains("page_1")) {
      ...
      if (el.classList.contains("duplicate")) {
      ...
      let inputNumber = parseInt(num[0], 10);

      if (isNaN(inputNumber)) {
        return "empty";
      }

      console.log("사용자 입력:", inputNumber);
      console.log("정답 설정:", rule);
      console.dir(rule);

      // 입력 값이 정답 배열에 포함되는지 확인
      if (rule.values.includes(String(inputNumber))) {
        return true; // 하나라도 맞으면 정답
      } else {
        return false; // 없으면 오답
      }
  ...
  window.onIncorrectCustom = function () {
    ...
    $(activePage)
      .find(".input_wrap math-field.hint.duplicate .text_hint")
      .css("opacity", "1");
  };
  ...
  window.onIncorrectTwiceCustom = function () {
    ...
    $(activePage)
      ...
      .css("opacity", "1");
    $(activePage)
      .find(".input_wrap math-field.hint + .text_hint")
      .css("opacity", "1");
  };
  ...
  window.resetCustom = function () {
    ...
    $(activePage)
      .find(".input_wrap math-field + .text_hint")
      .css("opacity", "0");
  };
});
