runAfterAppReady(() => {
    ...
    window.onIncorrectTwiceCustom = function () {
      ...
      // 현재 페이지의 오답 math-field에 대해 힌트 표시
      const page = pagenation.activePage;
      page.querySelectorAll("math-field[data-answer-multi]").forEach(mathField => {
        if (mathField.dataset.correction === "false") {
          const hintEl = mathField.closest(".input_wrap").querySelector(".multiple_hint");
          if (hintEl) {
            hintEl.classList.add("hint");
            hintEl.hidden = false;
          }
    ...
    // btnReset 클릭 시 multiple_hint와 hint 클래스 제거
    const originalResetCustom = window.resetCustom;
    window.resetCustom = function() {
      if (typeof originalResetCustom === "function") {
        originalResetCustom();
      }
      
      // 현재 페이지의 모든 multiple_hint 요소에서 hint 클래스 제거
      const page = pagenation.activePage;
      page.querySelectorAll(".multiple_hint").forEach(hintEl => {
