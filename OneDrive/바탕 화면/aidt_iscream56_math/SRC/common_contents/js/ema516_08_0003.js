...
runAfterAppReady(() => {
  ...
  window.getCustomTargets = function (page) {
    return $(page).find(".parallelogram");
  };
  ...
  window.customCheckCondition = function (el) {
    ...
    const $el = $(el);
    const top = $el.find(".line.top").hasClass("active");
    ...
    const hasAny = top || bottom || left || right;
  ...
  window.onIncorrectTwiceCustom = function () {
    ...
    currentPage.querySelectorAll(".parallelogram").forEach((el) => {
  ...
  window.resetCustom = function () {
    ...
    $(currentPage).find(".parallelogram").removeClass("hidden");
    $(currentPage).find(".answer_img").removeClass("active");
    $(currentPage).find(".parallelogram .line").removeClass("active");
  };
  ...
  defineButtonClassRules([
    {
      ...
      test: (el) => {
        ...
        if (!currentPage) return false;
        // 버튼 활성화 조건
        const val = $(el).closest(".parallelogram").find(".line.active");
  ...
  $(document).on("click", ".parallelogram .line", function (e) {
    e.stopPropagation();
