...
const createR2SmallDiv = (parent) => {
    ...
    div.addEventListener('click', () => {
        if (div.parentElement === r2) {
            ...
            if (!moved) {
                console.log('모든 wrap이 가득 찼습니다!');
            }
        } else {
            const currentWrap = div.parentElement;
            r2.appendChild(div);

            // 만약 원래 있던 wrap이 w1이 아니고, small_div가 다 없어졌으면 border 제거
            if (currentWrap.querySelectorAll('.small_div').length === 0) {
                currentWrap.style.border = 'none';
            }
        }
